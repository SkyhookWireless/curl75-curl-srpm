...
%global nss_ok %([ 0%{?fedora} -gt 15 -o 0%{?rhel} -gt 6 ] && echo 1 || echo 0)
...
# Use multilib-rpm-config where available
%global have_multilib_rpm_config %([ 0%{?fedora} -gt 21 -o 0%{?rhel} -gt 4 ] && echo 1 || echo 0)
...
Patch107:	0107-curl-7.50.2-libidn-valgrind.patch
...
# ssh-related tests 582, 600-641, 2004 fail for as-yet unknown reasons on F-12 to F-15
...
LD_LIBRARY_PATH="%{buildroot}%{_libdir}:$LD_LIBRARY_PATH" \
	make DESTDIR=%{buildroot} INSTALL="install -p" install -C scripts
...
if [ -z "$(find /etc/fedora-release -mtime +400)" -o 0%{?rhel} -gt 4 -o "%{?fedora}" = "12" ]; then
